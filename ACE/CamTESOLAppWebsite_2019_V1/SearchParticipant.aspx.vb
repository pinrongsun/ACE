...
"'" & txtfullname.Text.Trim & "','" & txtemail.Text.Trim & "','" & txtpassword_create.Text & "','" & dplCountry.SelectedValue & "',0,0,0,'" & RandomNumber & "','" & Now.ToString("yyyy-MM-dd H:mm") & "','',0")
    ...
    Protected Sub btnLogin0_Click(sender As Object, e As EventArgs) Handles btnSearch.Click
        ...
        cn.Open()
        Dim Fields As String = "GivenName + ' ' + Upper(FamilyName) as FullName, CountryofResidence, Position"
        Dim tbl As String = "tblRegistrationCamTESOL_2016"

        Dim Condition As String = ""
        If dplSearchFeild.SelectedValue = "GivenName" Then
            Condition = "Where GivenName like '%" & txtkeyword.Text.Trim & "%'"
        ElseIf dplSearchFeild.SelectedValue = "FamilyName" Then
            Condition = "Where FamilyName like '%" & txtkeyword.Text.Trim & "%'"
        ElseIf dplSearchFeild.SelectedValue = "Country" Then
            Condition = "Where CountryofResidence like '%" & txtkeyword.Text.Trim & "%'"
        ElseIf dplSearchFeild.SelectedValue = "Position" Then
            Condition = "Where Position like '%" & txtkeyword.Text.Trim & "%'"
        End If
        cmd.CommandText = "SELECT " & Fields & " FROM " & tbl & " " & Condition & " Order by GivenName, FamilyName ASC"
        ...
        gdSearch.DataSource = ds
        gdSearch.DataBind()
    End Sub

    Protected Sub gdSearch_DataBound(sender As Object, e As EventArgs) Handles gdSearch.DataBound
        Try

            gdSearch.HeaderRow.Cells(0).Font.Size = 10

            gdSearch.HeaderRow.Cells(1).Font.Size = 10
            gdSearch.HeaderRow.Cells(1).Text = "Country"
            gdSearch.HeaderRow.Cells(1).HorizontalAlign = HorizontalAlign.Left
            gdSearch.HeaderRow.Cells(1).Width = 80
            gdSearch.HeaderRow.Cells(2).Font.Size = 10
            gdSearch.HeaderRow.Cells(2).HorizontalAlign = HorizontalAlign.Left
            For i = 0 To gdSearch.Rows.Count - 1
                gdSearch.Rows(i).Cells(0).Font.Size = 8
                gdSearch.Rows(i).Cells(1).Font.Size = 8
                gdSearch.Rows(i).Cells(2).Font.Size = 8
            Next
        Catch ex As Exception
    ...
    Protected Sub gdSearch_SelectedIndexChanged(sender As Object, e As EventArgs) Handles gdSearch.SelectedIndexChanged
